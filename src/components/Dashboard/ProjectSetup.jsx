...
export default function ProjectSetup({ globalParams, setGlobalParams, onApply }) {
  ...
  // --- HELPER: HIGHLIGHT ZERO/EMPTY CELLS ---
  ...
  // --- 2. CALCULATIONS ---
  const valExt = parseFloat(extLen) || 0;
  const valInt = parseFloat(intLen) || 0;
  const customSum = customWalls.reduce((sum, w) => sum + (parseFloat(w?.length) || 0), 0);
  
  const structuralOpenLen = openAreas.reduce((sum, a) => {
      // Only include "Free Length" if it is marked as Structural/Plinth
  ...
  // Open Areas (UPDATED STRUCTURE & LOGIC)
  const addOpenArea = () => updateState({ 
      openAreas: [...openAreas, { id: Date.now(), type: 'Sit-out', touchingLen: '', freeLen: '', hasRoof: true, width: '2.5', roofType: 'RCC Slab' }] 
  });
  
  const updateOpenArea = (id, field, value) => {
      let updates = {};
      const cleanValue = (field === 'touchingLen' || field === 'freeLen' || field === 'width') ? handleMathInput(value) : value;

      updates[field] = cleanValue;

      // Logic: If TYPE changed, toggle includeInPlinth AND hasRoof defaults
      if (field === 'type') {
          // Defaulting Logic
          const shouldHaveRoof = (value !== 'Courtyard' && value !== 'Open Area');
          const isStructural = (value !== 'Car Porch' && value !== 'Open Area'); 
          
          updates.hasRoof = shouldHaveRoof;
          updates.includeInPlinth = isStructural;
      }
      
      updateState({ 
          openAreas: openAreas.map(a => a.id === id ? { ...a, ...updates } : a) 
      });
  };

  const togglePlinthInclude = (id) => {
      updateState({ openAreas: openAreas.map(a => a.id === id ? { ...a, includeInPlinth: !a.includeInPlinth } : a) });
  };
  const toggleRoofInclude = (id) => {
      updateState({ openAreas: openAreas.map(a => a.id === id ? { ...a, hasRoof: !a.hasRoof } : a) });
  };
  const removeOpenArea = (id) => updateState({ openAreas: openAreas.filter(a => a.id !== id) });

  // Columns & Floors (Kept as is)
  const addColumnGroup = (type) => {
    const isMain = type === 'Concealed';
    const defaults = getSuggestedSizes(numFloors);
    const newGroup = { id: Date.now(), type, name: isMain ? 'Concealed Column' : 'Open Column (RCC)', count: 1, f_l: isMain ? defaults.f.l : 1.0, f_b: isMain ? defaults.f.b : 1.0, f_d: isMain ? defaults.f.d : 1.2, c_l: isMain ? defaults.c.l : 0.23, c_b: isMain ? defaults.c.b : 0.23 };
  ...
  const handleFloorCountChange = (val) => {
    ...
    if (count > newNames.length) { for (let i = newNames.length; i < count; i++) newNames.push(i === 0 ? 'Ground Floor' : i === 1 ? 'First Floor' : 'Second Floor'); } 
    else { newNames.length = count; }
    
    const standards = getSuggestedSizes(count);
    const updatedColumns = columnGroups.map(grp => {
        if (grp.type === 'Concealed' || isRCC) return { ...grp, f_l: standards.f.l, f_b: standards.f.b, f_d: standards.f.d, c_l: standards.c.l, c_b: standards.c.b };
        return grp;
    });
            ...
            <div className="space-y-6">
                ...
                <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 shadow-sm relative">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                        {/* Ext Walls */}
                        <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm"><label className="block text-[10px] font-bold text-blue-500 uppercase tracking-wider mb-1">Ext. Length (m)</label><input type="number" className={`w-full p-2 border-b border-slate-200 outline-none text-lg font-bold text-slate-800 ${getZeroStyle(extLen)}`} value={extLen} onChange={(e) => updateState({extLen: e.target.value})} /><div className="flex items-center gap-2 mt-2 text-xs text-gray-500"><span>Width:</span><input type="number" className={`w-16 p-1 border rounded text-center ${getZeroStyle(extWidth)}`} value={extWidth} onChange={(e) => updateState({extWidth: e.target.value})} /></div></div>
                        {/* Int Walls */}
                        <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm"><label className="block text-[10px] font-bold text-green-500 uppercase tracking-wider mb-1">Int. Length (m)</label><input type="number" className={`w-full p-2 border-b border-slate-200 outline-none text-lg font-bold text-slate-800 ${getZeroStyle(intLen)}`} value={intLen} onChange={(e) => updateState({intLen: e.target.value})} /><div className="flex items-center gap-2 mt-2 text-xs text-gray-500"><span>Width:</span><input type="number" className={`w-16 p-1 border rounded text-center ${getZeroStyle(intWidth)}`} value={intWidth} onChange={(e) => updateState({intWidth: e.target.value})} /></div></div>
                    </div>
                    {/* Custom Walls */}
                            ...
                            <div key={area.id} className="flex flex-col gap-2 bg-white p-3 rounded-lg border border-orange-200 shadow-sm transition-all hover:shadow-md">
                                ...
                                <div className="flex justify-between items-center border-b border-orange-50 pb-2">
                                    ...
                                    <div className="flex items-center gap-3">
                                        {/* Plinth/Foundation Toggle */}
                                        <button 
                                            onClick={() => togglePlinthInclude(area.id)}
                                            className={`flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded border transition-colors ${area.includeInPlinth ? 'bg-green-50 text-green-700 border-green-200' : 'bg-gray-50 text-gray-400 border-gray-200'}`}
                                            title="Does this area need Foundation & Flooring?"
                                        >
                                            {area.includeInPlinth ? <CheckSquare size={12}/> : <Square size={12}/>}
                                            {area.includeInPlinth ? 'Structural' : 'Non-Structural'}
                                        </button>
                                        {/* Roofing Toggle */}
                                        <button 
                                            onClick={() => toggleRoofInclude(area.id)}
                                            className={`flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded border transition-colors ${area.hasRoof ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-gray-50 text-gray-400 border-gray-200'}`}
                                            title="Is this area roof-covered?"
                                        >
                                            {area.hasRoof ? <Home size={12}/> : <Square size={12}/>}
                                            {area.hasRoof ? 'Has Roof' : 'No Roof'}
                                        </button>
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    <div className="col-span-1">
                                        <label className="block text-[9px] font-bold text-gray-400 uppercase mb-1">Depth/Width (m)</label>
                                        <input type="text" placeholder="e.g. 2.5" className={`w-full p-1.5 bg-gray-50 border rounded text-center text-xs font-bold ${getZeroStyle(area.width)}`} value={area.width} onChange={(e) => updateOpenArea(area.id, 'width', e.target.value)} />
                                    </div>
                                    <div className="col-span-2">
                                        <label className="block text-[9px] font-bold text-gray-400 uppercase mb-1">Roof Type (Optional)</label>
                                        <select className={`w-full p-1.5 bg-gray-50 border rounded text-center text-xs font-bold ${!area.hasRoof ? 'opacity-50' : ''}`} value={area.roofType} onChange={(e) => updateOpenArea(area.id, 'roofType', e.target.value)} disabled={!area.hasRoof}>
                                ...
                                <div className="flex gap-4 border-t border-gray-100 pt-2 mt-2">
                                    <div className="flex-1">
                                        ...
                                        <input type="text" placeholder="e.g. 3" className={`w-full p-1.5 bg-gray-50 border rounded text-center text-xs font-bold ${getZeroStyle(evaluateMath(area.touchingLen))}`} value={area.touchingLen} onChange={(e) => updateOpenArea(area.id, 'touchingLen', e.target.value)} />
                                ...
                                <div className="flex justify-between mt-1 pt-1 border-t border-gray-50">
                                    <div className="text-[10px] font-bold text-gray-400">
                                        Roof Area: <span className="text-gray-700">{(evaluateMath(area.width) * (evaluateMath(area.touchingLen) + evaluateMath(area.freeLen))).toFixed(2)} m²</span>
                                    </div>
                                    <div className="text-[10px] font-bold text-gray-400">
                                        Total Perimeter: <span className="text-gray-700">{(evaluateMath(area.touchingLen) + evaluateMath(area.freeLen)).toFixed(2)} m</span>
                                    </div>
                                </div>
                            </div>
                        ))}
                ...
                {/* Total Foundation Length (Uses only Structural Free Length) */}
            ...
            <div className="space-y-6">
                <div className="bg-indigo-50/50 p-6 rounded-2xl border border-indigo-100 shadow-sm">
                    ...
                    <select className="w-full p-3 bg-white border border-indigo-100 rounded-xl font-bold text-indigo-700 mb-4" value={foundationType} onChange={(e) => updateState({foundationType: e.target.value, columnGroups: []})}>
                        <option value="RR">RR Masonry (Load Bearing)</option><option value="RCC">RCC Column Structure</option>
                    </select>
                    <div className="flex items-center gap-2"><label className="text-xs font-bold text-indigo-900">Floors:</label><input type="number" min="1" max="10" className="w-16 p-2 border rounded font-bold text-center" value={numFloors} onChange={(e) => handleFloorCountChange(e.target.value)} /><span className="text-[10px] text-indigo-500 bg-white px-2 py-1 rounded border ml-2">Auto-Updates Sizes</span></div>
                </div>
                ...
                <div className={`p-6 rounded-2xl border shadow-sm h-full ${isColumnMissing ? 'bg-red-50 border-red-200' : 'bg-green-50/50 border-green-100'}`}>
                    ...
                    <div className="mb-3 flex flex-col gap-1 text-[10px] text-gray-500 bg-white p-2 rounded border border-dashed"><div className="flex items-center gap-1 text-blue-500 font-bold"><Info size={12}/> Auto-Size Standards:</div><div><b>G+0:</b> Foot 1.0x1.0, Depth 1.2, Col 23x23cm</div><div><b>G+1:</b> Foot 1.2x1.2, Depth 1.5, Col 23x30cm</div><div><b>G+2:</b> Foot 1.5x1.5, Depth 1.5, Col 30x30cm</div><div><b>G+3:</b> Foot 1.8x1.8, Depth 1.8, Col 30x45cm</div></div>
